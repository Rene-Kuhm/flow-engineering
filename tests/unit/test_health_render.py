...
def test_render_text_mixed_verdicts_with_columns_and_rows() -> None:
    ...
    output = render_workspace_health_text(
        _sample_envelope(), console=Console(no_color=True, file=StringIO(), width=120)
    )
...
def test_render_text_ascii_ellipsis_no_unicode_ellipsis() -> None:
    ...
    output = render_workspace_health_text(
        envelope, console=Console(no_color=True, file=StringIO(), width=120)
    )
...
def test_render_text_custom_console_seam_returns_captured_string() -> None:
    ...
    buffer = StringIO()
    output = render_workspace_health_text(
        _sample_envelope(), console=Console(no_color=True, file=buffer, width=120)
    )
