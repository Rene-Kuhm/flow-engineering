...
Use this guide when hosted CI, release flow, or drift-detection behavior fails. The goal is to restore service, capture the cause, and prevent repeat failures without adding bureaucracy.

## Quick path

1. Identify the failing surface: hosted CI, release, CLI behavior, memory/SDD, or documentation drift.
2. Capture evidence before changing anything: run ID, command output, log excerpt, commit SHA, and timestamp.
3. Apply the smallest safe fix.
4. Verify with the narrowest relevant command first, then hosted CI when applicable.
...
| Verification | Local command, hosted CI run, or manual check. |
...
| SEV2 | CI or the release path is degraded but a workaround exists. | Fix in the next operational slice. |
...
- Keep Windows workflow scripts PowerShell-native unless bash is explicitly installed.
- Avoid `gh --jq` in Windows workflows when PowerShell can parse JSON directly.
- Every operational fix needs evidence: commit SHA plus hosted CI run ID.
