...
Slice 1 (T1.2a + T1.2b + T3.2) of the drift-detection change. The
Protocol seam replaces the inline graph-load plumbing inside
``decision_drift.scan_change`` so future slices (OTel push,
cross-project federation, per-finding ``graph_unavailable``) can plug
in without touching the pure classifier.

The 4 typed exceptions (``GraphLoadError`` + ``GraphMissing`` +
``GraphMalformed`` + ``PermissionDenied`` + ``SnapshotEnvelopeCorrupt``)
live in a dedicated ``drift_exceptions.py`` module (T3.2 GREEN) and are
re-imported here for backward-compat with the pre-T3.2 import path.
...
- ``SnapshotGraphLoader(snap_id: str)`` — concrete adapter for the
  snapshot-pinned path.
- ``_parse_envelope_graph`` + ``_index_graph_payload`` + ``_parse_line_for_loader``
  + ``_resolve_snapshots_dir_for_loader`` — co-located helpers.
...
# Re-export the typed exceptions for backward-compat with pre-T3.2 callers
# that did ``from flow_engineering.drift_graph_loader import GraphMissing`` etc.
__all__ = [
    "GraphLoadError",
    "GraphMissing",
    "GraphMalformed",
...
class LiveDiskGraphLoader:
    ...
    Raises ``GraphMissing`` (path absent), ``GraphMalformed`` (JSON
    decode / shape mismatch), or ``PermissionDenied`` (OSError with
    errno in {EACCES, EPERM, EROFS}) — replaces the bare
    ``return (None, None, None)`` fail-open at ``decision_drift.py:238-248``.
    """
...
def _parse_envelope_graph(envelope: dict) -> tuple[dict | None, dict | None, float | None]:  # type: ignore[type-arg]
    ...
    Mirrors the existing ``decision_drift._load_graph_from_snapshot:319-359``
    logic EXACTLY (returns ``(None, None, None)`` when no graph content
    is present — the D2 graceful degradation signal that surfaces as
    ``SnapshotGraphMissing`` at the scan boundary).
    """
...
def _index_graph_payload(
    ...
    Mirrors the existing ``decision_drift._index_graph_payload``
    (lines 252-274). Tolerates both ``file/line`` and
