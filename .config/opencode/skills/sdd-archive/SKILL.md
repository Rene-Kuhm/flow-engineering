...
When the change touches `decision-code-linking` (or its successor changes), the archived report MUST reference the `code_refs` block lineage so `flow inspect <change>` can still render the binding surface after archive. Build the block via `flow_engineering.binding.format_code_refs_block(refs, source=...)`; sources: `manual` / `auto_suggest` / `backfill` / `unbound`. The parser raises `ParseError` on malformed JSON. Observability counters (`inspect_invoked_total`, `backfill_observations_total`) feed the `flow metrics` summary and must be referenced in the archive report's verification table.
