...
## Decision ↔ Code Binding Hook

When the change touches `decision-code-linking` (or its successor changes), the verification MUST confirm that every Engram observation carries a valid trailing `code_refs` block parseable by `flow_engineering.binding.extract_code_refs`. The block is gated by `<!-- code_refs -->` followed by JSON, with sources `manual` / `auto_suggest` / `backfill` / `unbound`. Build it via `flow_engineering.binding.format_code_refs_block(refs, source=...)` or via `EngramClient.save_phase(phase, content, with_suggest=...)`. The parser raises `ParseError` on malformed JSON; one bad row MUST NOT blank the inspect table. Observability counters (`inspect_invoked_total`, `backfill_observations_total`, etc.) live in `flow_engineering.observability` and feed the `flow metrics` summary — a non-empty `flow metrics` output is part of the verification evidence.
