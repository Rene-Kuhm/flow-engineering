...
When the change touches `decision-code-linking` (or its successor changes), every apply-progress entry MUST include a trailing `code_refs` block (or pass `with_suggest=True` to `EngramClient.save_phase`) so `flow inspect <change>` can render the binding surface. Sources: `manual` / `auto_suggest` / `backfill` / `unbound`. Build the block via `flow_engineering.binding.format_code_refs_block(refs, source=...)`; the parser raises `ParseError` on malformed JSON. Observability counters (`inspect_invoked_total`, `bindings_confirmed_total`, `backfill_observations_total`) feed the `flow metrics` summary.
