...
When the change touches `decision-code-linking` (or its successor changes), artifacts persisted to Engram MUST carry a trailing `code_refs` block so `flow inspect <change>` can render the binding surface. The block is gated by `<!-- code_refs -->` followed by JSON, with sources `manual` / `auto_suggest` / `backfill` / `unbound`. Build it via `flow_engineering.binding.format_code_refs_block(refs, source=...)` or use `EngramClient.save_phase(phase, content, with_suggest=...)` for the auto-suggest path. The parser raises `ParseError` on malformed JSON; one bad row MUST NOT blank the inspect table. Observability counters (`inspect_invoked_total`, `backfill_observations_total`, etc.) live in `flow_engineering.observability` and feed the `flow metrics` summary.
