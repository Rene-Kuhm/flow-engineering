...
| **D10** | Prune safety gate | `flow snapshot prune` without `--confirm` → dry-run, prints JSON report `{would_delete: [snap_ids], would_keep: [snap_ids], freed_bytes_estimate: N}` to stdout, exits 0, **deletes nothing**. With `--confirm` → actually deletes. `--keep-last=0` requires BOTH `--confirm` AND `--force` (cannot be combined with `--keep-days` or `--max-total-size-mb`; refuses if both present). The combination `--keep-last=0 --force --confirm` is the only way to delete every snapshot. | Dry-run default mirrors D6 + `flow projects backfill`. Two-flag requirement for "delete everything" prevents the classic "I meant 1, not 0" foot-gun. JSON report makes the dry-run BDD-testable. |
...
| **8** | Prune default behavior | **Dry-run when no flags.** Exit 0, JSON `{would_delete, would_keep, freed_bytes_estimate}` to stdout, deletes nothing. User MUST pass at least one of `--keep-last`, `--keep-days`, `--max-total-size-mb` AND `--confirm` to actually delete. (Resolves Q8 from proposal as proposed.) |
