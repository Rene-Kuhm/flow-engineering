...
1. **D6 flat module names** — the design uses `src/flow_engineering/drift_graph_loader.py` (flat) instead of the locked spec's `src/flow_engineering/drift/_graph_loader.py` (package). The flat layout eliminates `drift/__init__.py` barrel boilerplate (~5 LOC saved). Tradeoff: the spec wording is now stale; a follow-up `drift-detection-spec-align` micro-change is recommended after Slice 1 verifies.
...
- **r7**: D6 deviation (flat module layout vs locked spec wording) creates review confusion. *Mitigation*: T0.1 PR body cites design.md §11 D6 explicitly + adds a brief note in the design's "Notes for sdd-tasks" section. The spec amendment is a follow-up `drift-detection-spec-align` micro-change after Slice 1 verifies.
...
- D6 spec/impl alignment — `drift-detection-spec-align` micro-change recommended AFTER Slice 1 verifies.
