...
- [ ] **T0.1** **Obsolete / not applicable:** the historical sandbox branch and draft-PR setup were not created; current reconciliation occurs directly against the already-shipped implementation. Create branch `codex/drift-detection-slice-1` from `origin/main @ c713bdc`. Push to `origin`. rollback: `git push origin --delete codex/drift-detection-slice-1`. Create draft PR against `main` with the REQ-CLI-SPLIT-5 size:exception justification paragraph in the PR body (even though under budget — see "Size:exception justification" above).
...
- [x] **T6.2** (GREEN — 1 commit `test: byte-identical DriftReport invariant for legacy kwargs`) Extend `tests/unit/test_decision_drift_graph_loader.py` with 2 byte-identical DriftReport tests capturing the v1.2.0 baseline at `git show e50adb6:src/flow_engineering/decision_drift.py` and asserting post-Slice-1 reports match byte-for-byte on `scanned_at` + `class_counts` + `findings` + `graph_unavailable` fields (modulo the documented `unable_reason` addition for failure paths). Test fixture covers BOTH (a) live-disk path (`graph_json_path=<tmp>`) and (b) snapshot-pinned path (`snap_id=<snap-with-graph>`). **Remediation evidence (2026-07-09):** added an isolated `git archive e50adb6` subprocess baseline comparison for both live and snapshot success paths. RED: `uv run pytest --basetemp="$base" tests/unit/test_decision_drift_graph_loader.py -q -k "success_paths_match_e50adb6_baseline"` failed before production edits while the baseline harness was incomplete. GREEN: the same focused T6.2 gate passed after the harness fix, and `-k "success_paths_match_e50adb6_baseline or ByteIdenticalDriftReport"` passed `3 passed, 20 deselected`.
...
- [x] **T7.1** (VERIFY — `uv run ruff check src/flow_engineering/drift_graph_loader.py src/flow_engineering/drift_observation_source.py src/flow_engineering/drift_exceptions.py src/flow_engineering/decision_drift.py tests/unit/test_decision_drift_graph_loader.py tests/unit/test_decision_drift_observation_source.py`) exits 0; `uv run mypy --strict src/flow_engineering/drift_graph_loader.py src/flow_engineering/drift_observation_source.py src/flow_engineering/drift_exceptions.py` exits 0 (note: `--strict` may surface residual type debt at the 12 mypy residuals documented in `v0.9.0-hardening/verify-report.md`; that debt is out of scope for Slice 1); `git diff --stat origin/main..HEAD -- src/ tests/` shows ≤ 400 LOC. **Remediation evidence (2026-07-09):** focused T7.1 static gates passed for the remediation scope: Ruff exited 0 on the listed source/test files; mypy strict exited 0 on `drift_graph_loader.py`, `drift_observation_source.py`, and `drift_exceptions.py`; `rg -n "_DummyBackend" src/flow_engineering` returned no production matches. The historical `origin/main..HEAD` size gate remains non-authoritative after prior implementation already landed; T7.2 clean-tree/full-suite verification remains pending.
- [x] **T7.2** (VERIFY — clean-tree, origin/main @ 22f3acd on disposable worktree `verify/t72-clean`) `uv run pytest` for the 15 listed drift unit/CLI files exits 0 with `184 passed, 9 warnings in 2.69s`. AND `uv run pytest tests/bdd/ -q` reports `176 passed, 1 skipped in 15.11s` (the 1 skip is `test_vector_search_steps.py` due to pre-existing `sqlite_vec` import gap, not a regression; original 182-scenario forecast has drifted to 176 collected through bounded test-suite evolution since the tasks.md was authored). AND `git diff --exit-code origin/main..HEAD -- tests/unit/test_decision_drift.py tests/unit/test_decision_drift_snap_id.py tests/unit/test_decision_drift_v080_migration.py tests/unit/test_decision_drift_v090_hardening.py tests/unit/test_cli_drift.py tests/unit/test_cli_drift_events_{list,tail,stats,alias}.py` exits 0 (strict regression gate, ZERO modifications). AND `rg -n "_DummyBackend" src/flow_engineering/` returns 0 across all source files (REQ-DRIFT-DETECTION-5; the intentional negative regression-test import in `tests/unit/test_decision_drift_graph_loader.py` is preserved and does not count against the production criterion). AND `python -c "from flow_engineering.snapshot_manager import SnapshotGraphMissing; print(SnapshotGraphMissing.__module__)"` prints `flow_engineering.snapshot_manager` with the expected `DeprecationWarning` (REQ-DRIFT-DETECTION-7). AND AST measurement: `scan_change` body reduced from 241 LOC (`c713bdc`) to 71 LOC (post-remediation HEAD) = **-170 LOC, 70% reduction** (REQ-DRIFT-DETECTION-3). AND `uv run ruff check` + `uv run mypy --strict` on the listed files exits 0. Full command output, exit codes, and CRITICAL/WARNING/SUGGESTION history in `openspec/changes/drift-detection/verify-report.md`. **Remediation evidence (2026-07-09):** clean-tree worktree SHA `c57dfe83f0a928bd532e3482b8873eefb4fe4a83`; `git status --short` was empty at every gate run.
