...
- **v1.2.0b** (2026-06-28) — `v1.2-followups` (#12) PR#2b archive sync. **REQ-V1.2.2** (alias REQ-48) **golden regression tests SHIPPED**: `render_prompt_canonical()` helper at `prompt_registry.py:1033` + 4 on-disk snapshot files at `tests/golden/prompts/{strict_tdd,auto_suggest_header,auto_suggest_footer,auto_suggest_empty}.txt` (119B / 29B / 61B / 37B) + `--update-goldens` + `--check-snapshot` Click flags on `flow prompts show <id>` (exit code `_EXIT_GOLDEN_DRIFT = 3` at `cli.py:3263` on drift) + `golden_snapshot_dir` + `production_golden_dir` fixtures extracted to `tests/unit/conftest.py:18-40`. 11 NEW tests in `tests/unit/test_prompt_render_golden.py` (4 `TestGoldenRegression` byte-match + 4 `TestCanonicalRenders` triangulation + 3 `TestGoldenUpdate` CliRunner integration). **1360/1360 tests passing** (+11 net vs v1.2.0a baseline of 1349; 0 regressions). `pyproject.toml` version still `1.1.0` (correct — PR#2d handles the BREAKING `1.1.0 → 1.2.0` bump + `## [1.2.0]` CHANGELOG entry); PR#2b CHANGELOG entry is `## [1.2.0b]` (pre-release marker, NOT v1.2.0). PR#2b is 2 of 4 chained PRs in the v1.2 release (stacked-to-main); PR#2c (REQ-54 `min_sdd_skill_versions`) + PR#2d (Path A rename + closeout) still pending. Verify verdict `PASS WITH WARNINGS` (0 CRITICAL + 0 WARNING + 3 SUGGESTION accepted per `drift-hardening` + `v0.9.0-hardening` + `v1.0-followups` + `v1.1-followups` + v1.2-followups PR#2a precedent; the 3 SUGGESTIONs are S1 missing `req48_golden_prompts.feature` BDD file + S2 `flow drift` graph unavailable + S3 `"TEST_COMMAND"` vs `"pytest"` sentinel divergence — all non-blocking). Full evidence at `openspec/changes/archive/2026-06-28-v1.2-followups-pr2b/verify-report-pr2b.md`.
- **v1.3 (2026-06-28) — `v1.1-followups` (change #11) archive sync.** All 3 deferred v1.1 REQs (REQ-51 prompt_renders.jsonl sink + REQ-52 prompt observability counters + REQ-53 docs/prompts.md auto-gen) SHIPPED. **REQ-51 SHIPPED** — NEW `prompt_render_log.py:200 LOC` module with `PromptRenderEvent` (frozen dataclass with `prompt_id` + `rendered_at` + `elapsed_ms` + `ok` + `error` + `var_keys`) + `PromptRenderLog` writer (with `_lock` per D11) + `record_prompt_render()` opt-in via `FLOW_PROMPT_LOG=1` env var (truthy values: `1`/`true`/`yes`/`on`) + best-effort `try/except OSError: pass` swallow. Sink path: `~/.flow-engineering/prompt_renders.jsonl` (mirrors `drift_events.jsonl` precedent at `~/.flow-engineering/drift_events.jsonl`). CLI surface: `flow prompts show <id> --render-count` returns rendered template + `render_count: <N> (last rendered_at: <iso> | never)`; `--render-history [N]` returns aligned text table of last N records. 16 NEW tests in `tests/unit/test_prompt_render_log.py` + 3 NEW instrumentation tests in `tests/unit/test_prompt_render.py::TestRenderPromptWritesToSinkWhenEnabled` + 8 NEW tests in `tests/unit/test_cli_prompts_show_render.py` (27 NEW total). **REQ-52 SHIPPED** — 3 NEW counters in `PROMPT_RENDER_COUNTER_NAMES` catalog at `observability.py:488-490`: `prompts_render_total{domain, prompt_id, status}` (always emitted) + `prompts_render_ms{domain, prompt_id, count}` (wall-clock duration via monotonic timer; emitted on success) + `prompts_render_failed_total{domain, prompt_id, error}` (emitted on failure). `DOMAIN_BY_PREFIX['prompts_'] = 'prompt'` extension at `observability.py:582` so `--domain prompt` filter works. `record_prompt_render_summary()` helper at `observability.py:507-554` emits 2 counters on success / 3 on failure. `render_prompt()` wrapped with `_render_started_monotonic = _time.monotonic()` timer at `prompt_registry.py:804` + `_emit_render_record()` helper at `prompt_registry.py:915-953` wires everything together. T4.4 refactor: `_prompt_domain_value: str = prompt.domain.value` (real domain, not hardcoded "unknown"). 10 NEW tests in `tests/unit/test_observability_prompt_counters.py`. **REQ-53 SHIPPED** — NEW `scripts/generate_prompts_doc.py:223 LOC` walks `PROMPT_NAMES` + reads each `.j2` template + renders via `render_prompt_safe()` (sentinel substitution per OQ-4) + emits Markdown. `docs/prompts.md:122 LOC` generated artifact with header + summary table (4 rows) + per-prompt sections (purpose + where it appears + example output + template body). `Makefile:31-32` `docs:` phony target calls `uv run python scripts/generate_prompts_doc.py`. Script is idempotent (`test_build_doc_is_idempotent` PASSES). 10 NEW tests in `tests/unit/test_generate_prompts_doc.py`. **REQ-48 / REQ-54 carry forward to v1.2** (unchanged from v1.1): REQ-48 golden regression tests via `tests/golden/prompts/<prompt_id>.txt` snapshots; REQ-54 `min_sdd_skill_versions: dict[str, str]` gate in `pyproject.toml`. Next change: `v1.2-followups` (#12) — Path A rename + remaining ruff residuals + planning-artifact backfill + REQ-48/REQ-54.
...
| **REQ-48** — golden regression tests | ✅ SHIPPED via `v1.2-followups` PR#2b (v1.2.0b, 2026-06-28) | `render_prompt_canonical()` helper at `prompt_registry.py:1033` + 4 snapshot files at `tests/golden/prompts/{strict_tdd,auto_suggest_header,auto_suggest_footer,auto_suggest_empty}.txt` (119B / 29B / 61B / 37B, UTF-8 + trailing newline) + `--update-goldens` + `--check-snapshot` Click flags on `flow prompts show <id>` (exit code 3 on drift) + `golden_snapshot_dir` + `production_golden_dir` fixtures extracted to `tests/unit/conftest.py:18-40`. 11 NEW tests in `tests/unit/test_prompt_render_golden.py` (4 TestGoldenRegression byte-match + 4 TestCanonicalRenders triangulation + 3 TestGoldenUpdate CliRunner integration). 1360/1360 tests pass (+11 net). Full evidence at `openspec/changes/archive/2026-06-28-v1.2-followups-pr2b/verify-report-pr2b.md`. |
...
| **REQ-51..54** — counters + sidecar + docs | ✅ REQ-51/52/53 SHIPPED via v1.1-followups (change #11, 2026-06-28); REQ-54 carries to v1.2 | **REQ-51** — NEW `prompt_render_log.py:200 LOC` module with `PromptRenderEvent` (frozen dataclass) + `PromptRenderLog` writer (`_lock` per D11) + `record_prompt_render()` opt-in via `FLOW_PROMPT_LOG=1` env var. Sink path: `~/.flow-engineering/prompt_renders.jsonl`. CLI: `flow prompts show <id> --render-count` + `--render-history [N]`. **REQ-52** — 3 NEW counters in `PROMPT_RENDER_COUNTER_NAMES`: `prompts_render_total{domain, prompt_id, status}` + `prompts_render_ms{domain, prompt_id, count}` + `prompts_render_failed_total{domain, prompt_id, error}`. `DOMAIN_BY_PREFIX['prompts_'] = 'prompt'` extension. `record_prompt_render_summary()` helper. `_render_started_monotonic` timer + `_emit_render_record()` helper. **REQ-53** — NEW `scripts/generate_prompts_doc.py:223 LOC` walks `PROMPT_NAMES` + `.j2` templates + emits `docs/prompts.md:122 LOC` (idempotent). `Makefile docs:` target. **REQ-54** (golden regression tests + `min_sdd_skill_versions`) — 🔲 NOT SHIPPED (v1.2 deferred). |

## v1.1.0 archive status (2026-06-28)

**v1.1-followups (change #11) SHIPPED as v1.1.0 — single PR, 6 sequential sub-batches (A + B + C + D + E + F) of strict TDD, 18 functional tasks complete (T1.1..T6.3) across 19 work-unit commits on `main` (HEAD `6cae060`).** 3 prompt-registry-related REQs (REQ-51 + REQ-52 + REQ-53) SHIPPED; REQ-54 carries to v1.2.

| REQ | Title | Status |
|-----|-------|--------|
| **REQ-V1.1.3** | `prompt_renders.jsonl` append-only sink + `PromptRenderEvent` frozen dataclass (`prompt_id` + `rendered_at` + `elapsed_ms` + `ok` + `error` + `var_keys`) + `PromptRenderLog` writer (with `_lock` per D11) + `record_prompt_render()` opt-in via `FLOW_PROMPT_LOG=1` (truthy: `1`/`true`/`yes`/`on`) + best-effort `try/except OSError: pass` + `flow prompts show <id> --render-count` + `--render-history [N]` flags | ✅ **SHIPPED** |
| **REQ-V1.1.4** | `prompts_render_total{domain, prompt_id, status}` + `prompts_render_ms{domain, prompt_id, count}` + `prompts_render_failed_total{domain, prompt_id, error}` counters + `PROMPT_RENDER_COUNTER_NAMES` catalog at `observability.py:488-490` + `DOMAIN_BY_PREFIX['prompts_'] = 'prompt'` extension at `observability.py:582` + `record_prompt_render_summary()` helper at `observability.py:507-554` (emits 2 counters on success / 3 on failure) + `_render_started_monotonic = _time.monotonic()` timer at `prompt_registry.py:804` + `_emit_render_record()` helper at `prompt_registry.py:915-953` (real `PromptDomain.value`, not hardcoded "unknown") | ✅ **SHIPPED** |
| **REQ-V1.1.5** | `scripts/generate_prompts_doc.py:223 LOC` walks `PROMPT_NAMES` + reads each `prompts/*.j2` template + renders via `render_prompt_safe()` (sentinel substitution per OQ-4) + emits `docs/prompts.md:122 LOC` (header + summary table + per-prompt sections: purpose + where it appears + example output + template body) + `Makefile:31-32` `docs:` phony target + idempotent (`test_build_doc_is_idempotent` PASSES) | ✅ **SHIPPED** |

**Verdict at archive**: **PASS WITH WARNINGS — archive-ready** (accepted per `drift-hardening` + `v0.9.0-hardening` + `v1.0-followups` precedent posture). Per `openspec/changes/archive/2026-06-28-v1.1-followups/verify-report.md`: **0 CRITICAL findings** + **3 WARNING** + **2 SUGGESTION** (none prompt-registry-specific; the 3 WARNINGs are doc-process + ruff tech-debt; the 2 SUGGESTIONs are a 1-line `prompt_render_log.py` trailing newline + positive docstring feedback on the intentional `SnapshotGraphMissing` parallel class). All 3 prompt-registry-related REQs (REQ-V1.1.3/4/5) have at least one passing test demonstrating compliance. **77 NEW v1.1 tests** across 6 NEW test files (16 in `test_prompt_render_log.py` + 3 in `test_prompt_render.py::TestRenderPromptWritesToSinkWhenEnabled` + 8 in `test_cli_prompts_show_render.py` + 10 in `test_observability_prompt_counters.py` + 10 in `test_generate_prompts_doc.py` + 30 in `test_drift_event_log.py` + 10 in `test_snapshot_graph_missing_error.py` = 87 across all v1.1; prompt-registry surface = 47). All 18 functional tasks (T1.1..T6.3) closed with strict-TDD RED → GREEN → REFACTOR evidence across 6 sub-batches. **1342/1342 tests passing** (+67 net vs v1.0 baseline). **182/182 BDD scenarios passing** (no NEW BDD scenarios — drift-events CLI surface BDD coverage was already complete from v1.0 follow-ups).

**Findings tally** (cross-cutting for the whole change, prompt-registry lens):
- **W1** (doc-process, RESOLVED by archive) — `openspec/specs/decision-drift/spec.md` v1.1 archive section NOT added at apply time (REQ-V1.1.6 T6.4 missing). **THIS ARCHIVE resolves W1** by adding the `## v1.1.0 archive status` sections to both `decision-drift/spec.md` + `prompt-registry/spec.md` + flipping the Versioning rows from PLANNED → SHIPPED.
...
- **W3** (tech-debt, ACCEPTED) — 17 ruff errors in v1.1-touched files (33 project-wide; 16 pre-existing in untouched files). The T6.3 `ruff --fix --unsafe-fixes` only fixed 3 of the 17 (UP022 + UP042 + C419 on `decision_drift.py`). Remaining 17 are deferred per the `v0.9.0-hardening` + `v1.0-followups` acceptable-residual-ruff precedent.
- **S1** (cleanup, ACCEPTED) — `prompt_render_log.py:200` missing trailing newline (W292, 1-line fix).
- **S2** (positive, KEEP) — `decision_drift.py:179` N818 `SnapshotGraphMissing` naming convention is intentional (parallel class for backwards compat with batch B1 BDD tests; documented in class docstring).

**Carry-forwards CLOSED** (prompt-registry lens):
- `v1.0-followups` **REQ-51** (prompt_renders.jsonl sink deferred) — closed via REQ-V1.1.3
- `v1.0-followups` **REQ-52** (prompt observability counters deferred) — closed via REQ-V1.1.4
- `v1.0-followups` **REQ-53** (docs/prompts.md auto-generated deferred) — closed via REQ-V1.1.5

**Carry-forwards NOT closed** (deferred to v1.2):
- **REQ-48** (golden regression tests via `tests/golden/prompts/<prompt_id>.txt` snapshots) — v1.2
- **REQ-54** (`min_sdd_skill_versions: dict[str, str]` gate in `pyproject.toml`) — v1.2
- **REQ-44** (`metrics.jsonl` rotation; was deferred beyond v1.1 per `v0.9.0-hardening` S2 carry-forward table) — v1.2
- **Path A subcommand group rename** for `flow drift` → `flow drift-events` (BREAKING for every existing caller; was deferred per `v1.0-followups` S4) — v1.2+
- **17 ruff residuals** in v1.1-touched files (4 auto-fixable + 10 hidden fixes + 3 intentional KEEP) — v1.2 tech-debt
- **W2 planning-artifact backfill** for changes that ran as commit-history-only (v1.1-followups is the first change in this posture) — v1.2 doc-process
...
## v1.2.0b archive status (2026-06-28)

**`v1.2-followups` (#12) PR#2b (v1.2.0b) SHIPPED as the second of 4 chained PRs in the v1.2 release — single PR, sub-batch B only, 6 functional tasks complete (T2.1..T2.6), 6 work-unit commits on `main` (HEAD `17cbf03` ahead of v1.2.0a baseline by 6 commits).**

| REQ | Title | Status |
|-----|-------|--------|
| **REQ-V1.2.2** (alias REQ-48) | Golden regression tests for prompts: `render_prompt_canonical(prompt_id, **vars)` helper in `prompt_registry.py:1033` (deterministic byte-for-byte render of each `PROMPT_NAMES` entry with sentinel substitutions via `_CANONICAL_DEFAULTS` map at `prompt_registry.py:1024`) + 4 snapshot files at `tests/golden/prompts/{strict_tdd,auto_suggest_header,auto_suggest_footer,auto_suggest_empty}.txt` (119B / 29B / 61B / 37B respectively, UTF-8 + trailing newline) + `--update-goldens` + `--check-snapshot` Click flags on `flow prompts show <id>` (default mode fails on drift with exit code `_EXIT_GOLDEN_DRIFT = 3` at `cli.py:3263`; `--check-snapshot` emits "snapshot drift detected" on stderr) + `golden_snapshot_dir` + `production_golden_dir` fixtures extracted to `tests/unit/conftest.py:18-40` for testability + `render_prompt_canonical` added to `__all__` export at `prompt_registry.py:615` | ✅ **SHIPPED** |

**Verdict at archive**: **PASS WITH WARNINGS — archive-ready** (accepted per `drift-hardening` + `v0.9.0-hardening` + `v1.0-followups` + `v1.1-followups` + `v1.2-followups` PR#2a precedent posture). Per `openspec/changes/archive/2026-06-28-v1.2-followups-pr2b/verify-report-pr2b.md`: **0 CRITICAL findings** + **0 WARNING** + **3 SUGGESTION** (all accepted per orchestrator brief; all non-blocking). The 1 in-scope REQ (REQ-V1.2.2 / REQ-48) has 11 passing tests demonstrating compliance (4 `TestGoldenRegression` byte-match tests + 4 `TestCanonicalRenders` triangulation tests + 3 `TestGoldenUpdate` CliRunner integration tests). All 6 functional tasks (T2.1..T2.6) closed with strict-TDD RED → GREEN → REFACTOR evidence (2 RED + 2 GREEN + 2 REFACTOR/CHANGELOG commits). **1360/1360 tests passing** (+11 net vs v1.2.0a baseline of 1349; 0 regressions). **182/182 BDD scenarios passing** (no regressions; no NEW pytest-bdd step glue for REQ-V1.2.2 — the spec scenarios in `tests/bdd/req48_golden_prompts.feature` were planned but deferred per S1; see findings). Ruff clean on changed files (`prompt_registry.py` + `cli.py` + `tests/unit/conftest.py` + `tests/unit/test_prompt_render_golden.py` + `tests/golden/`). Spec/design drift check: 12/12 spec scenarios PASS + 6/7 design decisions EXACT (1 NOTE for sentinel value `"TEST_COMMAND"` vs plan's `"pytest"` — intentional sentinel hygiene; the implementation chose a clearly-sentinel value so operators can distinguish it from a real test command; byte-match contract holds for all 4 snapshots).

**Findings tally**: **0 CRITICAL + 0 WARNING + 3 SUGGESTION** (accepted per precedent; all non-blocking):
- **S1** (doc-process, ACCEPTED) — `tests/bdd/req48_golden_prompts.feature` not delivered. `proposal.md:123`, `tasks.md:36,447`, `design.md:107` all promise a NEW BDD feature file with 2 scenarios for REQ-V1.2.2; the file does not exist in `tests/bdd/`. Unit tests provide full coverage (11/11 pass) and the orchestrator's verify checklist did not include the BDD feature. Non-blocking. Optional follow-up: add `req48_golden_prompts.feature` in PR#2c closeout or v1.3 if BDD coverage is required for REQ-V1.2.2.
- **S2** (infra, ACCEPTED) — `flow drift v1.2-followups` returns `unable_to_verify` (exit 2). `~/.flow-engineering/graph.json` is not present for this project. This is environmental (PR#2a hit the same condition; resolves post-archive when the decision graph is seeded). Non-blocking. Optional follow-up: run `flow drift v1.2-followups --write-back` after archive to seed the graph for future drift scans.
- **S3** (doc-reference, NO-FIX-NEEDED) — `render_prompt_canonical` sentinel value is `"TEST_COMMAND"`, not `"pytest"`. Per `explore.md:85` + `tasks.md:186`, the plan named `"pytest"` as the canonical default. The implementation at `prompt_registry.py:1025` chose `"TEST_COMMAND"` (uppercase, clearly a sentinel) instead. **Intentional divergence**: the sentinel is more obvious as a placeholder than `"pytest"` (which could be mistaken for a real test command). The byte-match contract holds for all 4 snapshots. Test at `test_prompt_render_golden.py:99` verifies `"TEST_COMMAND"` is substituted. Non-blocking; consider updating `explore.md` / `tasks.md` to reflect the actual sentinel choice.
