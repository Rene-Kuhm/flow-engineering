...
| **v1.2** | **PLANNED** | **`v1.2-followups` (#12)** | **🔲 DEFERRED** | **Carry-forwards from v1.1: REQ-44 `metrics.jsonl` rotation (if not already covered) + REQ-48 golden regression tests for prompts + REQ-54 `min_sdd_skill_versions` gate in pyproject + Path A subcommand group rename for `flow drift-events` (BREAKING) + remaining 17 ruff residuals in v1.1-touched files (4 auto-fixable + 10 hidden fixes) + W2 backfill of on-disk planning artifacts (proposal.md + design.md + tasks.md + apply-progress/) for changes that ran as commit-history-only** |
