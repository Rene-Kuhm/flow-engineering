...
| **v1.2** | **PLANNED (chained remainder)** | **`v1.2-followups` (#12) PR#2b/c/d** | **🔲 DEFERRED** | **Remaining carry-forwards from v1.1 after PR#2a (REQ-44 closed): REQ-48 golden regression tests for prompts + REQ-54 `min_sdd_skill_versions` gate in pyproject + Path A subcommand group rename for `flow drift-events` (BREAKING in v1.2.0d) + remaining 17 ruff residuals in v1.1-touched files (4 auto-fixable + 10 hidden fixes) + W2 backfill of on-disk planning artifacts for v1.1-followups. 3 chained PRs remaining (PR#2b = REQ-48 golden tests, PR#2c = REQ-54 skill versions, PR#2d = Path A rename + closeout).** |
