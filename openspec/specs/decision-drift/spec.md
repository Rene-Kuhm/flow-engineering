...
<!-- Archive sync: 2026-07-09 (drift-detection change, T7.2 PASS; 8 ADDED Requirements appended from delta spec). -->
## ADDED Requirements (drift-detection change, 2026-07-09)

**Change**: `drift-detection` (Slice 1 of 3 candidate slices � GraphLoader + ObservationSource Protocols extraction).
**Archive source**: `openspec/changes/archive/2026-07-09-drift-detection/`.
**Verdict**: PASS (0 CRITICAL + 0 WARNING + 1 SUGGESTION at archive; SUGGESTION is bounded test-suite evolution since tasks.md was authored, not a regression).
**Scope**: structural refactor of `scan_change` over 2 narrow Protocols (`GraphLoader`, `ObservationSource`) + typed exception hierarchy (`GraphMissing` / `GraphMalformed` / `PermissionDenied` / `SnapshotEnvelopeCorrupt`) + `_DummyBackend` removal + `SnapshotGraphMissing` canonical relocation to `snapshot_manager.py` + `unable_reason` population from typed exceptions. **NO MODIFIED Requirements, NO REMOVED Requirements** � pure additive extension to the `decision-drift` capability. The 8 REQs below describe WHAT the seam must expose; they are pure structural contracts.
**Pre-archive HEAD**: `9521593` on `main` (commit `9521593 docs(drift): T7.2 PASS with clean-tree full verification evidence`).
