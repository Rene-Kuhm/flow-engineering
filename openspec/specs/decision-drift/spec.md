...
| **v1.1.0** | **PLANNED** | **`v1.1-followups` (#11)** | **🔲 DEFERRED** | **Carry-forwards from v1.0 follow-ups: DriftEventLog rotation (`FLOW_DRIFT_EVENT_LOG_MAX_BYTES` + `FLOW_DRIFT_EVENT_LOG_MAX_AGE_DAYS` env vars, alongside metrics rotation) + S1 wire-format hardening (drop defensive `str→int` shim; WARN becomes hard error) + REQ-51 (`prompt_renders.jsonl` sink) + REQ-52 (`flow prompt-events` observability counters) + REQ-53 (`docs/prompts.md` auto-generated) + 12 ruff `--unsafe-fixes` cleanup at `decision_drift.py`** |
...
**v1.1 entry — change #10 CLOSED + v1.0 SHIPPED (2026-06-28).** Change #11 (`v1.1-followups`) is the next change in the loop. Scope: DriftEventLog rotation + REQ-51/52/53 (prompt_renders.jsonl + flow prompt-events counters + docs/prompts.md auto-gen) + S1 wire-format hardening (drop the v1.0 defensive `str→int` shim — the WARN becomes a hard error in v1.1, requiring operators to run the CHANGELOG v1.0 1-line `sed` migration before upgrade) + 12 ruff `--unsafe-fixes` cleanup. The v1.0 hard-break is documented in `CHANGELOG.md:6-38` + `openspec/changes/archive/2026-06-28-v1.0-followups/verify-report.md`.
